...
void cancelLastActivitySingle(LibrarySingle *lib, infotype borrowerName) {
    ...
    while (qNode != NULL) {
        Queue *Q = &qNode->queue;
        address book = qNode->book;
        address borrower = Q->head;
        address prev = NULL;

        while (borrower != NULL && borrower != book) {
            if (strcmp(borrower->info, borrowerName) == 0) {
                infotype bookTitle = book->info;

                HistoryNode *historyNode = findHistory(&globalHistory, borrowerName);
                if (historyNode != NULL) {
                    char activity[100];
                    sprintf(activity, "Tidak jadi meminjam buku %s", bookTitle);
                    push(historyNode->activityStack, activity);
                }

                if (prev == NULL) {
                    Q->head = next(borrower);
                } else {
                    next(prev) = next(borrower);
                }
                if (borrower == Q->tail) {
                    Q->tail = prev;
                    if (Q->tail != NULL) {
                ...
                free(borrower->info);
                if (borrower->activityStack != NULL) {
                    deallocateStack(borrower->activityStack);
                    free(borrower->activityStack);
                }
                free(borrower);
                printf("%s telah membatalkan aktivitas terakhir mereka untuk buku %s.\n", borrowerName, bookTitle);
                return;
            }
            prev = borrower;
    ...
    printf("Peminjam %s tidak ditemukan dalam antrian mana pun.\n", borrowerName);
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: {
                ...
                scanf("%[^\n]", borrowerName);
                getchar();
                cancelLastActivitySingle(lib, borrowerName); 
                break;
            }
            case 7:
...
void cancelLastActivityDouble(LibraryDouble *lib, infotype borrowerName) {
    dAddress book = lib->bookList;
    while (book != NULL) {
        Queue Q;
        createQueue(&Q);
        Q.head = book->queueHead;
        Q.tail = book->queueHead;
        while (Q.tail != NULL && next(Q.tail) != NULL) {
            Q.tail = next(Q.tail);
        }
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: {
                ...
                scanf("%[^\n]", borrowerName);
                getchar();
                cancelLastActivityDouble(lib, borrowerName); 
                break;
            }
            case 7:
                printf("Kembali ke menu utama.\n");
...
void cancelLastActivityArray(LibraryArray *lib, infotype borrowerName) {
    for (int i = 0; i < lib->size; i++) {
        if (lib->bookList[i].info == NULL) continue;

        Queue Q;
        createQueue(&Q);
        Q.head = lib->bookList[i].queueHead;
        Q.tail = lib->bookList[i].queueHead;
        while (Q.tail != NULL && next(Q.tail) != NULL) {
            Q.tail = next(Q.tail);
        }
        ...
        while (borrower != NULL) {
            if (strcmp(borrower->info, borrowerName) == 0) {
                infotype bookTitle = lib->bookList[i].info;
                ...
                lib->bookList[i].queueHead = Q.head;
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: {
                ...
                scanf("%[^\n]", borrowerName);
                getchar();
                cancelLastActivityArray(lib, borrowerName); 
                break;
            }
            case 7:
                printf("Kembali ke menu utama.\n");
